...
def watcher(program, updates):
	...
	if stored_requirement != current_requirement:
		updates['programs/' + program + '/page-state'] = current_requirement
		if stored_requirement is not None:
			emailHelper(program, stored_requirement, current_requirement)

	extensions = db.child('programs').child(program).child(
		'extensions').get().val()
	for e in list(extensions or []):
		e_url = db.child('programs').child(program).child(
			'extensions').child(e).child('metadata').child(
				'catalog').get().val()
		e_xpath = db.child('programs').child(program).child(
			'extensions').child(e).child('metadata').child(
				'source').get().val()
		e_stored_requirement = db.child('programs').child(program).child(
			'extensions').child(e).child('page-state').get().val()
		e_tree = html.fromstring(requests.get(e_url).content)
		e_current_requirement = tree.xpath(e_xpath)[0].text_content()
		if e_stored_requirement != e_current_requirement:
			updates['programs/' + program + 'extensions/' + e + '/page-state' = e_current_requirement
			if e_stored_requirement is not None:
...
def emailHelper(program, stored_requirement, current_requirement):
	...
	diff = " ".join([
		'<b>' + i[2:] + '</b>' if i[:1] == '+' else i[2:] for i in d
		if not i[:1] in '-?'
	])
