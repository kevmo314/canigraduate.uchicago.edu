...
public class Scraper extends PTransform<PBegin, PDone> {
    ...
    public PDone expand(PBegin input) {
        ...
        PCollection<KV<TermKey, Course>> courses = PCollectionList.of(p.apply(new TimeschedulesTransform()))
        ...
        PCollection<TermKey> courseKeys = courses.apply("Fetch course keys", Keys.create());
        PCollectionList.of(p.apply(new FirestoreListCoursesTransform()).apply(new FirestoreListTermsTransform()))
                .and(courseKeys)
        ...
        PCollectionList.of(courseKeys.apply("Strip term and department",
                MapElements.into(TypeDescriptors.strings()).via(TermKey::getCourse))
