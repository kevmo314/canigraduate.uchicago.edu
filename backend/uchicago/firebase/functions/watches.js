...
function enrollmentDiff(previous, current) {
  const change = previous.val()[0] != current.val()[0] ||
...
function generateDiff(previous, current) {
  let [changed, sectionDiff] =
      enrollmentDiff(previous.child('enrollment'), current.child('enrollment'));
  let output = 'Section enrollment: \n\t' + sectionDiff;
  const previousActivities =
      Object.keys(previous.child('secondaries').val() || {});
  const currentActivities =
      Object.keys(current.child('secondaries').val() || {});
...
module.exports =
    functions.database.ref('/schedules/{course}/{year}/{period}/{section}')
        .onWrite(event => {
