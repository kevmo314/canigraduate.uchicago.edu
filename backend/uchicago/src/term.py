...
class Term(object):
    ...
    def all():
        timeschedules = bs4.BeautifulSoup(
            requests.get('http://timeschedules.uchicago.edu/browse.php').text,
            'lxml')
        for option in timeschedules.find(
                'select', {'id': 'term_name'}).find_all('option'):
            if option.has_attr('value'):
                term = TimeSchedulesTerm(
                    id=option.getText(), timeschedules_id=option['value'])
                if term.parsable:
                    yield term
        coursesearch = bs4.BeautifulSoup(
            requests.get(
        ...
        for option in coursesearch.find(
            ...
            if option.has_attr('value') and option['value']:
                yield CourseSearchTerm(
                    id=option.getText(), coursesearch_id=option['value'])

    @property
    def ordinal(self):
        return 4 * int(self.id[-4:]) + [
...
class CourseSearchTerm(Term):
    ...
    @property
    def department_uris(self):
        return CourseSearch(self.id, self.timeschedules_id).department_uris
...
class TimeSchedulesTerm(Term):
    ...
    @property
    def department_uris(self):
        return TimeSchedules(self.timeschedules_id).department_uris
