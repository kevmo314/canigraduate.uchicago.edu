...
export class SearchResultsComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.filters && this.filters) {
      const subsets: Observable<
        Set<string> | ((_: Set<string>) => Set<String>)
      >[] = [];
      if (
        this.filters.periods.length === environment.institution.periods.length
      ) {
        ...
        subsets.push(
          ...[
            Observable.forkJoin(
              this.filters.periods.map(period => {
      ...
      if (this.filters.instructors.size > 0) {
        this.filters.instructors.forEach(instructor => {
          subsets.push(
            this.databaseService.indexes('instructors/' + instructor).first()
          );
        });
      }
      if (this.filters.departments.size > 0) {
        this.filters.departments.forEach(department => {
          subsets.push(
            this.databaseService.indexes('departments/' + department).first()
          );
        });
      }
        ...
        subsets.push(
          this.databaseService.scheduleIndex().first().map(schedules => {
            return (state: Set<string>) => {
              ...
              // the provided state.
              return new Set(
                Array.from(state).filter(course => matchingCourses.has(course))
              );
            };
          })
        );
  ...
  getShown(course: string): Observable<boolean> {
    return Observable.combineLatest(
      this.store.select(s => s.shown.has(course)),
      this.countChange
    )
