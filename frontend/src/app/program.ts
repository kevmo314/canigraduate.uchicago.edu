...
export class Program extends Node {
  ...
  private async evaluateChild(requirement: Node | Leaf, state: Set<string>): Promise<{ progress: number, total: number }> {
    if (requirement.hasOwnProperty('requirement')) {
      // This is a leaf.
      await this.evaluateLeaf(<Leaf>requirement, state);
      return { progress: (requirement.force || (<Leaf>requirement).satisfier) ? 1 : 0, total: 1 };
    }
    return this.evaluateNode(<Node>requirement, state);
  }
  ...
  private async evaluateNode(node: Node, state: Set<string>): Promise<{ progress: number, total: number }> {
    ...
    for (let i = 0; i < node.requirements.length; i++) {
      let result = await this.evaluateChild(<Node | Leaf>node.requirements[i], state);
      if (i < (node.min || node.requirements.length)) {
        total += result.total;
      }
      progressions.push(result);
      if (result.progress === result.total && (++childrenCompleted === (node.max || node.requirements.length))) {
        break;
      }
    }
    progressions.sort((a, b) => b.progress - a.progress);
    node.total = total;
    node.progress = node.force ? node.total : progressions.reduce((sum, x) => sum + x.progress, 0);
    ...
    for (let i = 0; i < node.requirements.length; i++) {
      if (typeof node.requirements[i] === 'string') {
        ...
        if (requirement.startsWith('/')) {
          promises.push(this.databaseService.sequence(requirement.substring(1)).first().toPromise().then(graft => {
  ...
  constructor(private databaseService: DatabaseService) { super(); }
